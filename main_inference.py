...
def parse_arguments():
    ...
    parser.add_argument('--pretrained_root',  default="/home/cmz/experiments/ADNet-main/results_abd/train/EP2-fold0-supp0/model_0.763.pth", type=str)
    parser.add_argument('--save_root', default="/home/cmz/experiments/ADNet-main/results_abd/test/fold0", type=str)
    parser.add_argument('--fold', default=0, type=int)
    parser.add_argument('--dataset', default="CHAOST2", type=str)

    parser.add_argument('--n_shot', default=1, type=int)            # EP1 => 3  EP2 => 1
    parser.add_argument('--EP1', default=False, type=bool)          # 确认是否使用 EP1
...
def infer(model, query_loader, support_sample, args, logger, label_name):
    ...
    for i, sample in enumerate(query_loader):
        ...
        # visual_query_pred = query_pred.cpu().numpy()
        # visual_query_pred = visual_query_pred * 255.0
        # query_num = query_pred.shape[0]

        # visual_query_label = query_label[0].cpu().numpy()
        # visual_query_label = visual_query_label * 255.0

        # for i in range(query_num):
        #     visual_query_label_temp = Image.fromarray(np.uint8(visual_query_label[i]))
        #     visual_query_label_temp.save("/home/cmz/experiments/ADNet-main/visualization/visual_query_label_{}.jpg".format(i))
        #     visual_query_pred_temp = Image.fromarray(np.uint8(visual_query_pred[i]))
        #     visual_query_pred_temp.save("/home/cmz/experiments/ADNet-main/visualization/visual_query_pred_{}.jpg".format(i))
