...
class TestDataset(Dataset):
    ...
    def __init__(self, args):
        ...
        self.image_dirs = sorted(self.image_dirs, key=lambda x: int(x.split('_')[-1].split('.nii.gz')[0]))   # 20个 image 的 绝对路径

        # remove test fold!
        self.FOLD = get_folds(args.dataset)  # Chaos 是将20个拆成 5组 每组5个 [0,1,2,3,4] / [4,5,6,7,8] / [8,9,10,11,12] / [12,13,14,15,16] / [16,17,18,19,0]
        self.image_dirs = [elem for idx, elem in enumerate(self.image_dirs) if idx in self.FOLD[args.fold]]  # fold-n 中n个作为test的 nii.gz
    ...
    def __getitem__(self, idx):
        ...
        lbl = sitk.GetArrayFromImage(
            sitk.ReadImage(img_path.split('image_')[0] + 'label_' + img_path.split('image_')[-1]))  # test 使用的是 label
        lbl[lbl == 200] = 1
        lbl[lbl == 500] = 2
        lbl[lbl == 600] = 3
...
class TrainDataset(Dataset):
    ...
    def __init__(self, args):
        self.n_shot = args.n_shot                          # 1
        self.n_way = args.n_way                            # 1
        self.n_query = args.n_query                        # 1
        self.n_sv = args.n_sv                              # 5000
        self.max_iter = args.max_iterations                # 1000
        self.read = True                                   # read images before get_item
        self.train_sampling = 'neighbors'                  # sampling 策略
        self.min_size = 200                                # 最小数量

        # reading the paths (leaving the reading of images into memory to __getitem__)
        if args.dataset == 'CMR':
            self.image_dirs = glob.glob(os.path.join(args.data_root, 'cmr_MR_normalized/image*'))
        elif args.dataset == 'CHAOST2':
            self.image_dirs = glob.glob(os.path.join(args.data_root, 'chaos_MR_T2_normalized/image*'))         # 一共20个 所有的 图像 的绝对路径
        self.image_dirs = sorted(self.image_dirs, key=lambda x: int(x.split('_')[-1].split('.nii.gz')[0]))     # 排个序
        self.sprvxl_dirs = glob.glob(os.path.join(args.data_root, 'supervoxels_' + str(args.n_sv), 'super*'))  # 一共20个 所有的 superpix 的绝对路径
        self.sprvxl_dirs = sorted(self.sprvxl_dirs, key=lambda x: int(x.split('_')[-1].split('.nii.gz')[0]))   # 排个序

        # remove test fold!
        self.FOLD = get_folds(args.dataset)   # Chaos 是将20个拆成 5组 每组5个 [0,1,2,3,4] / [4,5,6,7,8] / [8,9,10,11,12] / [12,13,14,15,16] / [16,17,18,19,0]
        self.image_dirs = [elem for idx, elem in enumerate(self.image_dirs) if idx not in self.FOLD[args.fold]]    # fold-n 中的 n个 作为test
        self.sprvxl_dirs = [elem for idx, elem in enumerate(self.sprvxl_dirs) if idx not in self.FOLD[args.fold]]  # fold-n 中的 n个 作为test
    ...
    def __getitem__(self, idx): 
        ...
        # visual_sprvxl = sprvxl * 255.0 
        # for i in range(31):
        #     temp = Image.fromarray(np.uint8(visual_sprvxl[i]))
        #     temp.save("/home/cmz/workspace/ADNet-main/visualization/visual_sprvxl-{}.jpg".format(i))
        ...
        # 可视化 img & label
        # from PIL import Image
        # visual_supp_lbl = sup_lbl[0][0] * 255.0 
        # visual_supp_lbl = Image.fromarray(np.uint8(visual_supp_lbl))
        # visual_supp_lbl.save("/home/cmz/workspace/ADNet-main/visualization/supp_lbl.jpg")
        
        # visual_supp_img = sup_img[0][0][0] * 255.0
        # visual_supp_img = Image.fromarray(np.uint8(visual_supp_img))
        # visual_supp_img.save("/home/cmz/workspace/ADNet-main/visualization/supp_img.jpg")
